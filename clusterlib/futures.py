...
def _invalidate_nfs_folder_cache(folder):
    """Try to invalid the NFS negative-entry cache under Linux

    Call chown on the parent folder without changing the id of the
    owner. Note: this strategy is not garanteed to work on all File
    Systems but has proved to help with NFSv3 clients under Linux.

    http://shodan.theblah.org/~cras/nfs-coding-howto.html#fhcache

    """
    s = os.stat(folder)
    os.chown(folder, s.st_uid, s.st_gid)
...
class AtomicMarker(object):
    ...
    def isset(self):
        ...
        such as NFS.
        """
        _invalidate_nfs_folder_cache(self.job_folder)
        return op.isdir(self.marker_path)
    ...
    def set(self):
        ...
        try:
            ...
            if getattr(e, 'errno', None) == errno.EEXIST:
                # The marker folder was already set or by a concurrent
                # process.
                return False
            else:
    ...
    def unset(self):
        ...
        Return True if the marker was unset by us and False if it was
        already unset or unset concurrently by another process.
...
class ClusterExecutor(object):
    ...
    def _get_finished_future(self, job_folder, job_name, fn, args, kwargs,
        ...
        _invalidate_nfs_folder_cache(job_folder)
...
def execute_job(job_folder):
    ...
    # Put the running marker into this job folder. This marker also serves as
